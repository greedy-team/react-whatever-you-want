...
describe("SearchScreen", () => {
  ...
  it("검색하면 첫 번째 결과가 화면에 뜬다", async () => {
    ...
    expect(await screen.findByText(mockMalePersons[0].nm)).toBeInTheDocument();
  });
  ...
  it("다음 버튼을 누르면 다음 사람이 뜨고, 첫 결과에서는 이전 버튼이 비활성화된다", async () => {
    ...
    expect(await screen.findByText(mockMalePersons[1].nm)).toBeInTheDocument();
  });
  ...
  it("이전 버튼을 누르면 이전 사람이 뜬다", async () => {
    ...
    expect(await screen.findByText(mockMalePersons[5].nm)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "▲ 이전" }));
    expect(await screen.findByText(mockMalePersons[4].nm)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "▲ 이전" }));
    expect(await screen.findByText(mockMalePersons[3].nm)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "▲ 이전" }));
    expect(await screen.findByText(mockMalePersons[2].nm)).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: "▲ 이전" }));
    expect(await screen.findByText(mockMalePersons[1].nm)).toBeInTheDocument();
  });
