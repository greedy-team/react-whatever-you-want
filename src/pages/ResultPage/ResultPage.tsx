    ...
    return (
        <ResultContainer>
            <ResultCard>
                ...
                <InfoBlock>
                    <HighlightText>{UI_GENRES.find(g => g.key === genre)?.name || "전체"}</HighlightText>
                    
                    <InfoText>📅 선택한 시대</InfoText>
                    <HighlightText>{UI_ERAS.find(e => e.key === era)?.name || "전체"}</HighlightText>
                ...
                <SecondaryButton onClick={()=>navigate("/")}>🔄 필터 다시 고르기</SecondaryButton>
            </ResultCard>
        </ResultContainer>
    );
}
